...
모든 스크립트는 `skills/video/reels-editor/scripts/` 디렉토리에 있습니다:
...
```bash
python skills/video/reels-editor/scripts/generate_reels.py \
  --video [영상경로] \
  --mode auto \
  --output output/영상/
```
...
```bash
python skills/video/reels-editor/scripts/resize_video.py \
  --video [입력] \
  --output [출력]
```
...
```bash
# 단일 구간
python skills/video/reels-editor/scripts/trim_video.py \
  --video [입력] \
  --start [시작초] \
  --end [종료초] \
  --output [출력]
...
```bash
python skills/video/reels-editor/scripts/add_overlay.py \
  --video [입력] \
  --headline "[헤드라인]" \
  --cta "[CTA]" \
  --output [출력]
...
```bash
python skills/video/reels-editor/scripts/analyze_video.py \
  --video [입력] \
